...
<body>
    ...
    <div class="ml-[228px] p-5">
        ...
        <form class="flex justify-start">
            ...
            <div class="float-right mb-4 ml-auto flex justify-end">
                <select name="year" class="focus:shadow-outline w-full appearance-none rounded-lg border leading-tight text-gray-700 shadow focus:outline-none" id="filterAngkatan" onchange="this.form.submit()">
                    <option value="" selected>Pilih Angkatan</option>
                    @foreach ($years as $year)
                        <option value="{{ $year }}" @selected(request()->query('year') === $year)>{{ $year }}</option>
                    @endforeach
                </select>
                @error('year')
                    <p>{{ $message }}</p>
