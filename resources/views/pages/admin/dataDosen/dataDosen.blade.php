...
<body>
    ...
    <div class="ml-[228px] p-5">
        ...
        <div class="relative mt-2 overflow-x-auto">
            <table class="w-full text-left text-sm text-black rtl:text-right">
                <thead class="bg-[#1F5F92] text-xs uppercase text-black">
                    <tr>
                ...
                <tbody class="border-1 border border-black">
                    ...
                    @foreach ($lectures as $key => $lecture)
                        <tr class="border-1 border border-black bg-white">
                            <th scope="row" class="whitespace-nowrap px-2 py-1 text-center font-medium text-gray-900">{{ $loop->iteration }}</th>
                            <td class="px-6 py-4">{{ $lecture->lectureData->nip }}</td>
                            <td class="px-6 py-4">{{ $lecture->lectureData->nidn }}</td>
                            <td class="px-6 py-4">{{ $lecture->name }}</td>
                            <td class="px-6 py-4">{{ $lecture->email }}</td>
                            <td class="px-6 py-4">{{ $lecture->lectureData->kk }}</td>
                            <td class="flex gap-2 px-6 py-4">
                                @if (auth()->user()->id === $lecture->id)
                                    <div>
                                        <a href="/DataDosen/Edit?id={{ $lecture->id }}">
                                    ...
                                    <div class="div">
                                        <button type="button" onclick="document.getElementById('id-data-delete').setAttribute('value', '{{ $lecture->id }}')" data-modal-target="popup-modal1" data-modal-toggle="popup-modal1">
    ...
    <div id="popup-modal" tabindex="-1" class="fixed left-0 right-0 top-0 z-50 hidden h-[calc(100%-1rem)] max-h-full w-full items-center justify-center overflow-y-auto overflow-x-hidden backdrop-blur md:inset-0">
        <div class="relative max-h-full w-full max-w-md p-4">
            <div class="relative rounded-lg bg-white shadow">
