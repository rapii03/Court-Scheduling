    ...
    <aside id="default-sidebar"
        class="fixed top-0 left-0 z-30 w-[228px] h-screen transition-transform -translate-x-full sm:translate-x-0"
        aria-label="Sidebar">
        <div class="h-full py-6 bg-[#1F5F92]">
            <div class="mb-5">
                <a href="#">
                    <div class="text-center m-auto justify-center flex text-black text-[30px] font-bold">Scheduler <span
                            class="text-white">TI</span>
                    </div>
                </a>
            </div>
            <div class="flex items-center justify-center m-auto text-white">
                <ul class='w-[85%]'>
                    <div class="overflow-y-auto h-[500px] hide-scrollbar mt-10">
                        <li class="flex items-center justify-center inset-x-0 bg-[#EBEBEB] rounded-lg">
                            ...
                            @endphp
                            <a href="/DashboardUser" type="button"
                                class="group w-full h-[45px] p-1 pl-5 flex gap-6 text-lg font-medium
        @if (in_array(Request::path(), $activePagePath2)) rounded-lg bg-white text-[#C8AC5E]
        @else
        text-black rounded-lg hover:bg-white hover:text-[#C8AC5E] @endif">
                                <div class="text-start justify-center items-center flex">
                                    <svg width="13" height="14" viewBox="0 0 13 14" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M12.3837 7.22897L6.67821 1.63117L0.972756 7.22897L0.452118 6.49364L6.67821 0.385254L12.9043 6.49367L12.3837 7.22897ZM11.0603 13.1931L7.7629 13.1931V8.5081H5.59353V13.1931H2.29609V5.10604H3.10961V12.2358H4.78002V7.55071H8.57642V12.2358H10.2468V5.10604H11.0603L11.0603 13.1931Z"
                                            fill="currentColor" />
                                    </svg>
                                </div>
                                <div class="text-start justify-center items-center flex text-[14px] font-bold">Home
                                </div>
                            </a>
                        </li>
                        <li class="flex items-center justify-center inset-x-0 mt-2 bg-[#EBEBEB] rounded-lg">
                            ...
                            @endphp
                            <a href="/ProfileUser" type="button"
                                class="group w-full h-[45px] p-1 pl-5 flex gap-6 text-lg font-medium
        @if (in_array(Request::path(), $activePagePath3)) rounded-lg bg-white text-[#C8AC5E]
        @else
            text-black rounded-lg hover:bg-white hover:text-[#C8AC5E] @endif">
                                <div class="text-start justify-center items-center flex">
                        ...
                        <li class="flex items-center justify-center inset-x-0 mt-2 bg-[#EBEBEB] rounded-lg">
                            ...
                            @endphp
                            <a href="/DaftarSidang" type="button"
                                class="group w-full h-[45px] p-1 pl-5 flex gap-6 text-lg font-medium
        @if (in_array(Request::path(), $activePagePath4)) rounded-lg bg-white text-[#C8AC5E]
        @else
            text-black rounded-lg hover:bg-white hover:text-[#C8AC5E] @endif">
                                <div class="text-start justify-center items-center flex">
                                    <svg width="14" height="14" viewBox="0 0 14 14" fill="none"
                        ...
                        <li class="flex items-center justify-center inset-x-0 mt-2 bg-[#EBEBEB] rounded-lg">
                            ...
                            @endphp
                            <a href="/JadwalSidang" type="button"
                                class="group w-full h-[45px] p-1 pl-5 flex gap-6 text-lg font-medium
        @if (in_array(Request::path(), $activePagePath5)) rounded-lg bg-white text-[#C8AC5E]
        @else
            text-black rounded-lg hover:bg-white hover:text-[#C8AC5E] @endif">
                                <div class="text-start justify-center items-center flex">
                        ...
                        <hr class="bottom-0 left-0 w-full border-white mt-10">
                    </div>
                    <li class="absolute w-full -inset-x-0 bottom-0">
                        <a type="button" id="logOut" href="/"
                            class="btn border-[#F0394F] gap-3 w-40 py-1 mb-5 flex items-center justify-center m-auto rounded-md bg-white hover:bg-[#D51717] text-[#F0394F] hover:text-white p-2 pt-2 pb-2 font-bold">
                            Keluar
                        </a>
                    </li>
                </ul>
